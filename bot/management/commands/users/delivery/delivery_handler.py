...
async def delivery_task(call: CallbackQuery, state: FSMContext):
...
@dp.callback_query_handler(lambda callbask_query: callbask_query.data.startswith("delivery:"))
async def change_basket(callbask_query: CallbackQuery, state: FSMContext):
    create_task(delivery_task(callbask_query, state))
